...
var simon = (function(){
  ...
  function addButton(i, hue, tone, position, positionSpan){
    ...
    buttons[i] = {
      tone: tone,
      $el: $('<div/>', { 'class':'btnTarget', 'css':{'background-color':bgColor} })
    };
  ...
  function userPress (event) {
    if(isUserTurn){
      console.log("user turn " + userMove + ", seq is " + seq[userMove] + ", user clicked " + event.data.id );
      if ( event.data.id == seq[userMove] ){
  ...
  function userPressCorrect (event) {
    ...
    buttons[id].$el.addClass('pressed').bind('mouseup mouseleave', userRelease);
    userMove++;
  }
  ...
  function userRelease (event) {
    ...
    if (isUserTurn && userMove == seq.length) {
      userTurnComplete();
    }
  }

  function userTurnComplete(){
    userMove = 0;
    isUserTurn = false;
