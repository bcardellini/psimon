...
var simon = function () {
  ...
  if (document.documentElement.style.pointerEvents !== '') {
    document.getElementById('psiControls').innerHTML = "<br>sorry,<br>" + "this browser lacks a feature<br>" + "required by psimon<br><br>";
  }
  ...
  if (!audioContext) {
    alert("try Chrome, Firefox, or Safari to play this game with sound");
  } else {
  ...
  function incrementDifficulty(event) {
    ...
    if (newDiff >= maxDifficulty || newDiff <= minDifficulty) {
  ...
  function generateButtons() {
    ...
    for (var i = 0; i < buttonCount; i++) {
      var btnHue = (hueSeed + i * hueSpan) % 360;
      var btnTone = minTone + i * toneSpan;
      var btnPosition = 0.1 * Math.floor(10 * (i * positionSpan));
  ...
  function addButton(i, hue, tone, position, positionSpan) {
    var outerTransform = 'translateX(100%) rotate(' + position + 'deg) ' + 'translateX(' + gapPercent + '%)';
    var innerTransform = 'translateX(-' + gapPercent + '%) ' + 'rotate(' + 0.1 * Math.ceil(10 * (positionSpan - 180)) + 'deg) ' + 'translateX(' + gapPercent + '%)';
    ...
    $btnsContainer.append($('<div/>', { 'class': 'btnOuter', 'css': { 'transform': outerTransform } }).append($('<div/>', { 'class': 'btnInner', 'css': { 'transform': innerTransform } }).append(buttons[i].$el)));
  }
  ...
  function startSound(tone) {
    var shape = arguments.length <= 1 || arguments[1] === undefined ? 'square' : arguments[1];
  ...
  function userPressCorrect(event, id) {
    ...
    buttons[id].$el.addClass('pressed').bind('mouseup mouseleave touchend', userReleaseCorrect);
    userMove++;
  }
  ...
  function userPressIncorrect(event, id) {
    ...
    buttons[id].$el.addClass('wrong').bind('mouseup mouseleave touchend', userReleaseIncorrect);
  }

  function userReleaseCorrect(event) {
    stopSound();
    $(event.target).removeClass('pressed').off('mouseup mouseleave touchend');
  ...
  function userReleaseIncorrect(event) {
    ...
    $(event.target).removeClass('wrong').off('mouseup mouseleave touchend');
    if ($strictIn.prop('checked')) {
      //srict mode, end game
      $msg.text('game over').css('display', 'block').delay(2000).fadeOut(2000);
      resetGameplay();
    } else {
      // replay this level's sequence
      endUserTurn();
